package com.prunn.rfdynhud.widgets.prunn.f109.racetimingtower;
...
public class RaceTimingTowerWidget extends Widget
{
    ...
    private final ImagePropertyWithTexture imgPosFinish = new ImagePropertyWithTexture( "imgPos", "prunn/f109/finished.png" );
    ...
    protected void initialize( LiveGameData gameData, boolean isEditorMode, DrawnStringFactory drawnStringFactory, TextureImage2D texture, int width, int height )
    {
        ...
        imgPosFinish.updateSize( rowHeight, rowHeight, isEditorMode );
        ...
        { 
            dsPos[i] = drawnStringFactory.newDrawnString( "dsPos", imgPos.getTexture().getWidth()/2 + fontxposoffset.getValue(), top + fontyoffset.getValue(), Alignment.CENTER, false, getFont(), isFontAntiAliased(), whiteFontColor );
            dsName[i] = drawnStringFactory.newDrawnString( "dsName", imgPos.getTexture().getWidth()+10 + fontxnameoffset.getValue(), top + fontyoffset.getValue(), Alignment.LEFT, false, getFont(), isFontAntiAliased(), blackFontColor );
            dsTime[i] = drawnStringFactory.newDrawnString( "dsTime", imgPos.getTexture().getWidth() + imgName.getTexture().getWidth() + imgTime.getTexture().getWidth()*5/6 + fontxtimeoffset.getValue(), top + fontyoffset.getValue(), Alignment.RIGHT, false, getFont(), isFontAntiAliased(), whiteFontColor );
    ...
    protected void drawBackground( LiveGameData gameData, boolean isEditorMode, TextureImage2D texture, int offsetX, int offsetY, int width, int height, boolean isRoot )
    {
        ...
        for(int i=0;i < drawncars;i++)
        {
            ...
            {
                if(scoringInfo.getGamePhase() == GamePhase.SESSION_OVER && i==0)
                    texture.clear( imgPosFinish.getTexture(), offsetX, offsetY+rowHeight*i, false, null );
                else   
                    if(i==0)
    ...
    protected void drawWidget( Clock clock, boolean needsCompleteRedraw, LiveGameData gameData, boolean isEditorMode, TextureImage2D texture, int offsetX, int offsetY, int width, int height )
    {
        ...
        if ( needsCompleteRedraw)
        {
            ...
            { 
                ...
                if(gameData.getScoringInfo().getGamePhase() == GamePhase.SESSION_OVER && i==0)
    ...
    public void loadProperty( PropertyLoader loader )
    {
        ...
        if ( loader.loadProperty( fontxposoffset ) );
        if ( loader.loadProperty( fontxnameoffset ) );
        if ( loader.loadProperty( fontxtimeoffset ) );
    }
